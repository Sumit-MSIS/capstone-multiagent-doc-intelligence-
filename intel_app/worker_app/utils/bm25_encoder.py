...
class CustomBM25Encoder(BaseSparseEncoder):
    ...
    def custom_encode_documents(
        self, avgdl, texts: Union[str, List[str]]
    ) -> Union[SparseVector, List[SparseVector]]:
        """
        encode documents to a sparse vector (for upsert to pinecone)
        ...
        """
        if avgdl is None:
            raise ValueError("avgdl is missing!")

        if isinstance(texts, str):
            return self.custom_encode_single_document(avgdl, texts)
        
        elif isinstance(texts, list):
            return [self.custom_encode_single_document(avgdl, text) for text in texts]
        
        else:
            raise ValueError("texts must be a string or list of strings")
        

    def custom_encode_single_document(self, avgdl, text: str) -> SparseVector:
        indices, doc_tf = self._tf(text)
        tf = np.array(doc_tf)
        tf_sum = sum(tf)

        tf_normed = tf / (
            self.k1 * (1.0 - self.b + self.b * (tf_sum / avgdl)) + tf
        )
    ...
    def count_values_across_lists(self, lists: List[List[str]]) -> Dict[str, int]:
        """Calculate document frequency: number of docs each term appears in."""
        df_counter = Counter()
        for lst in lists:
            unique_terms = set(lst)  # ensure no duplicates even if input is messy
            df_counter.update(unique_terms)
        return dict(df_counter)


    def custom_doc_query(self, query:str, doc_freq: Dict, N: int) -> SparseVector:
        """Calculate the number of documents that contain the query."""
        try:
            indices, query_tf = self._tf(query)
            df = np.array([doc_freq.get(str(idx), 1) for idx in indices])
            ...
            #print(f"Error calculating doc_query: {str(e)}")
            return None


    def _recalculate_indices_scores(self, avgdl, indices, values) -> SparseVector:
        try:
            tf = np.array(values)
            tf_sum = sum(tf)
            tf_normed = tf / (
            ...
            # print(len(indices),len(tf_normed))
            # print(tf_normed)
            res = {
                "indices": indices,
                "values": tf_normed.tolist(),
            ...
            print(f"error in _recalculate_indices_scores {e}")
            raise
