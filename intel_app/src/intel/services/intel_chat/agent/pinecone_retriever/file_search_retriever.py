...
def reciprocal_rank_fusion(sparse_results, dense_results, k: int):
    try:
...
async def run_sparse_query_async(index, org_id, custom_filter, top_k, query, keyword_query, file_id, tag_ids):
    try:
        if keyword_query:
            sparse_vec = calculate_idf(keyword_query, org_id, [file_id], tag_ids)
        else:
            sparse_vec = calculate_idf(query, org_id, [file_id], tag_ids)
...
async def get_context_from_pinecone(
    ...
    try:
        ...
        # Run sparse + dense search in parallel
        sparse_task = run_sparse_query_async(
            sparse_index, org_id, custom_filter, top_k, query,
            keyword_search_query, file_id, tag_ids
        )

        dense_task = run_dense_query_async(
        ...
        # RRF Merge
        rrf_results = reciprocal_rank_fusion(sparse_res, dense_res, k=60)
