                    ...
                    CREATE VIEW file_metadata_view_{org_id} AS
                        WITH metadata_pivot AS (
                            SELECT 
                                ...
                                MAX(CASE WHEN fm.metadata_title = 'Parties Involved' THEN CAST(fm.metadata_text_value AS CHAR CHARACTER SET utf8mb4) END) AS party_name,
								...
								MAX(CASE WHEN fm.metadata_title = 'Contract Duration' THEN CAST(SUBSTRING_INDEX(fm.metadata_text_value, ' ', 1) AS DECIMAL(10,1)) END) AS contract_duration, -- contract duration is currently stored as a string of X months in file_metadata table so while creating view , only extracting the numerical part. 
                                MAX(CASE 
                        ...
                        FROM (
                            ...
                            AND is_archived IS NULL AND type = 1 AND name not like 'temp/%'
                        ) f
