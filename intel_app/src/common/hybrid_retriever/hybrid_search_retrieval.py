from langchain_openai import OpenAIEmbeddings
from pinecone import Pinecone
import time
from src.common.hybrid_retriever.reranking import rerank_documents,pinecone_rerank_documents
from src.config.base_config import config
from src.common.logger import _log_message
import json
import orjson
import mlflow
import requests
from src.common.file_insights.hybrid_vector_handler.bm25_handler import calculate_idf
from src.common.file_insights.hybrid_vector_handler.bm25_reindexing import BM25ReIndexing
from concurrent.futures import ThreadPoolExecutor, as_completed
from opentelemetry import context as ot_context

mlflow.openai.autolog()
MODULE_NAME = "hybrid_search_retrieval.py"

# this mapping is required to map dense index with its associated sparse index
# after the implementation of separate indexing for hybrid retrieval there will be two individual indexes, one for sparse and one for dense
# to not affect the get_context_from_pinecone() method's parameters , we are using this mapping approach to get the corresponding sparse index.
INDEX_MAPPING = {
    config.DOCUMENT_SUMMARY_AND_CHUNK_INDEX: config.DOCUMENT_SUMMARY_AND_CHUNK_INDEX_SPARSE,
    config.DOCUMENT_SUMMARY_INDEX: config.DOCUMENT_SUMMARY_INDEX_SPARSE,
    config.DOCUMENT_METADATA_INDEX: config.DOCUMENT_METADATA_INDEX_SPARSE
}

@mlflow.trace(name="Get Hybrid Pinecone Index")
def get_pinecone_index(index_name, api_key, logger):
    """Fetch the Pinecone index."""
    try:
        start_time = time.perf_counter()
        pinecone_client = Pinecone(api_key=api_key)
        index = pinecone_client.Index(index_name)

        logger.debug(_log_message(f"Fetching Pinecone index '{index_name}'.", "get_pinecone_index", MODULE_NAME))
        logger.info(_log_message(f"Pinecone index '{index_name}' connected successfully.", "get_pinecone_index", MODULE_NAME))

        execution_time = round(time.perf_counter() - start_time, 6)
        logger.debug(_log_message(f"Index fetch time: {execution_time} seconds.", "get_pinecone_index", MODULE_NAME))

        return index
    except Exception as e:
        logger.error(_log_message(f"Error connecting to Pinecone index '{index_name}': {e}", "get_pinecone_index", MODULE_NAME))
        raise

@mlflow.trace(name="Get Hybrid Vectors")
def get_embeddings(org_id, file_id, query, logger) -> dict:
    """Generate dense and sparse embeddings for the query."""
    try:
        start_time = time.perf_counter()

        embeddings = OpenAIEmbeddings(
            api_key=config.OPENAI_API_KEY,
            model=config.OPENAI_EMBEDING_MODEL_NAME
        )
        dense_embedding = embeddings.embed_query(query)
        org_str = f"org_id_{org_id}"


        execution_time = round(time.perf_counter() - start_time, 6)

        logger.debug(_log_message(f"Embeddings generated for query: '{query}'.", "get_embeddings", MODULE_NAME))
        logger.info(_log_message(f"Embeddings computed in {execution_time} seconds.", "get_embeddings", MODULE_NAME))

        return dense_embedding
    except Exception as e:
        logger.error(_log_message(f"Error generating embeddings for query '{query}': {e}", "get_embeddings", MODULE_NAME))
        return None


@mlflow.trace(name="Hybrid Score Normalization")
def hybrid_score_norm(dense, sparse, alpha: float):
    """Hybrid score using a convex combination

    alpha * dense + (1 - alpha) * sparse

    Args:
        dense: Array of floats representing
        sparse: a dict of `indices` and `values`
        alpha: scale between 0 and 1
    """
    if alpha < 0 or alpha > 1:
        raise ValueError("Alpha must be between 0 and 1")
    
    hs = {
        'indices': sparse['indices'],
        'values':  [v * (1 - alpha) for v in sparse['values']]
    }
    
    return [v * alpha for v in dense], hs

########-------------------------------------------------------------------------------
# using RRF on both retrievals
###------------------------------------------------------------------------------------
@mlflow.trace(name="Reciprocal Rank Fusion")
def reciprocal_rank_fusion(sparse_results, dense_results, k:int, logger) -> dict:
    """
    Combines sparse and dense search results using Reciprocal Rank Fusion (RRF).

    sparse_results, dense_results = dict of matches from Pinecone
    """
    try:
        logger.info(_log_message(f"Initializing Reciprocal rank fusion","reciprocal_rank_fusion",MODULE_NAME))
        if "matches" in sparse_results:
            sparse_matches = sparse_results.get("matches",[])  
        else:
            sparse_matches = []
            logger.error(_log_message("matches key not found in sparse resultfs","reciprocal_rank_fusion",MODULE_NAME))
        
        if "matches" in dense_results :
            dense_matches = dense_results.get("matches",[]) 
        else:
            dense_matches = []
            logger.error(_log_message("matches key not found in dense results","reciprocal_rank_fusion",MODULE_NAME))

        @mlflow.trace(name="RRF - Rank Results")
        def rank_results(results):
            """Return a dict: id -> RRF score."""
            scores = {}
            for rank, item in enumerate(results, start=1):
                rrf_score = 1 / (k + rank)
                scores[item["id"]] = (rank,rrf_score,item['score'])
            return scores

        # Rank each list
        sparse_scores = rank_results(sparse_matches)
        dense_scores = rank_results(dense_matches)

        # Merge scores
        combined_scores = {}
        all_ids = set(sparse_scores.keys()) | set(dense_scores.keys())
        # print(all_ids)
        # print(len(all_ids))

        for chunk_id in all_ids:
            # combined_scores[chunk_id] = sparse_scores.get(chunk_id, 0) + dense_scores.get(chunk_id, 0)
            sparse_rank, sparse_rrf,sparse_score = sparse_scores.get(chunk_id, (0, 0, 0))
            dense_rank, dense_rrf,dense_score = dense_scores.get(chunk_id, (0, 0, 0))
            combined_scores[chunk_id] = {
                "rrf_score": sparse_rrf + dense_rrf,
                "sparse_rank": sparse_rank,
                "dense_rank": dense_rank,
                "sparse_score":sparse_score,
                "dense_score":dense_score
            }

        # Build combined result list
        id_to_metadata = {}
        for res in sparse_matches+ dense_matches:
            id_to_metadata[res["id"]] = res

        fused_results = [
            {
                "id": chunk_id,
                "rrf_score": data["rrf_score"],
                "sparse_rank": data["sparse_rank"],
                "dense_rank": data["dense_rank"],
                "metadata": id_to_metadata[chunk_id]["metadata"]
            }
            for chunk_id, data in sorted(combined_scores.items(), key=lambda x: x[1]["rrf_score"], reverse=True)
        ]
        logger.info(_log_message("Reciprocal Rank Fusion Complete","reciprocal_rank_fusion",MODULE_NAME))
        return fused_results
    except Exception as e:
        logger.error(_log_message(f"Error in RRF {e}","reciprocal_rank_fusion",MODULE_NAME))
        RuntimeError(f"Reciprocal Rank Fusion failed: {e}")



def submit_with_context(executor, fn, *args, **kwargs):
    """Submit a function to executor with OpenTelemetry context propagation."""
    parent_ctx = ot_context.get_current()
    def wrapped():
        token = ot_context.attach(parent_ctx)
        try:
            return fn(*args, **kwargs)
        finally:
            ot_context.detach(token)
    return executor.submit(wrapped)


@mlflow.trace(name="Retrieve Context from Pinecone")
def get_context_from_pinecone(
    pinecone_dense_index_name,
    pinecone_api_key,
    custom_filter,
    top_k,
    query,
    file_id,
    user_id,
    org_id,
    tag_ids=None,
    logger=None,
    keyword_search_query=None):
    """
    Hybrid search retrieval using Pinecone with dual indexing (sparse + dense).
    Runs sparse & dense retrievals in parallel and merges them via RRF.
    """
    try:
        start_time = time.perf_counter()
        logger.debug(_log_message(f"[{org_id}] Initiating hybrid search retrieval.", "get_context_from_pinecone", MODULE_NAME))

        # Map to sparse index internally
        sparse_index_name = INDEX_MAPPING.get(pinecone_dense_index_name)
        if not sparse_index_name:
            raise ValueError(f"Unsupported Pinecone sparse index name for {pinecone_dense_index_name}")

        dense_index = get_pinecone_index(pinecone_dense_index_name, pinecone_api_key, logger)
        sparse_index = get_pinecone_index(sparse_index_name, pinecone_api_key, logger)

        # ----------------------
        # Define parallel tasks
        # ----------------------
        @mlflow.trace(name="Run Sparse Query")
        def run_sparse_query():
            try:
                if pinecone_dense_index_name != config.DOCUMENT_SUMMARY_AND_CHUNK_INDEX:
                    return "sparse", ({}, 0.0)

                if keyword_search_query:
                    sparse_vectors = calculate_idf(keyword_search_query, org_id, file_id if isinstance(file_id, list) else [file_id], tag_ids, logger)
                else:
                    sparse_vectors = calculate_idf(query, org_id, file_id if isinstance(file_id, list) else [file_id], tag_ids, logger)

                if not sparse_vectors:
                    logger.error(_log_message(f"[{org_id}] No Sparse Indices Created", "get_context_from_pinecone", MODULE_NAME))
                    return "sparse", ({}, 0.0)

                start = time.perf_counter()
                result = sparse_index.query(
                    sparse_vector=sparse_vectors,
                    namespace=f"org_id_{org_id}#",
                    top_k=top_k,
                    filter=custom_filter,
                    include_metadata=True,
                    include_values=False
                )
                exec_time = round(time.perf_counter() - start, 6)
                logger.info(_log_message(f"[{org_id}] Sparse query executed in {exec_time} seconds.", "get_context_from_pinecone", MODULE_NAME))
                return "sparse", (result, exec_time)
            except Exception as e:
                logger.error(_log_message(f"[{org_id}] Sparse query failed: {e}", "get_context_from_pinecone", MODULE_NAME))
                return "sparse", ({}, 0.0)
            
        @mlflow.trace(name="Run Dense Query")
        def run_dense_query():
            try:
                dense_embedding = get_embeddings(
                    org_id, file_id if isinstance(file_id, list) else [file_id], query, logger
                )
                if not dense_embedding:
                    logger.error(_log_message(f"[{org_id}] No dense Embeddings Created", "get_context_from_pinecone", MODULE_NAME))
                    return "dense", ({}, 0.0)

                start = time.perf_counter()
                result = dense_index.query(
                    vector=dense_embedding,
                    namespace=f"org_id_{org_id}#",
                    top_k=top_k,
                    filter=custom_filter,
                    include_metadata=True,
                    include_values=False
                )
                exec_time = round(time.perf_counter() - start, 6)
                logger.info(_log_message(f"[{org_id}] Dense query executed in {exec_time} seconds.", "get_context_from_pinecone", MODULE_NAME))
                return "dense", (result, exec_time)
            except Exception as e:
                logger.error(_log_message(f"[{org_id}] Dense query failed: {e}", "get_context_from_pinecone", MODULE_NAME))
                return "dense", ({}, 0.0)

        # ----------------------
        # Run both in parallel
        # ----------------------
        with ThreadPoolExecutor(max_workers=2) as executor:
            futures = [
                submit_with_context(executor, run_sparse_query),
                submit_with_context(executor, run_dense_query)
            ]
            results = {name: val for name, val in (f.result() for f in as_completed(futures))}

        sparse_results, sparse_time = results.get("sparse", ({}, 0.0))
        dense_results, dense_time = results.get("dense", ({}, 0.0))

        if not sparse_results or not sparse_results.get("matches"):
            logger.error(_log_message(f"[{org_id}] No sparse results found for query '{query}'.", "get_context_from_pinecone", MODULE_NAME))
            
        if not dense_results or not dense_results.get("matches"):
            logger.error(_log_message(f"[{org_id}] No dense results found for query '{query}'.", "get_context_from_pinecone", MODULE_NAME))

        no_dense = not dense_results or not dense_results.get("matches")
        no_sparse = not sparse_results or not sparse_results.get("matches")

        if no_dense and no_sparse:
            logger.error(_log_message(
                f"[{org_id}] No results found for query '{query}'.",
                "get_context_from_pinecone",
                MODULE_NAME
            ))
            return {
                "matches": [],
                "namespace": f"org_id_{org_id}#",
                "usage": {"read_units": 0}
            }

        # ----------------------
        # Merge & rerank
        # ----------------------
        results = reciprocal_rank_fusion(sparse_results, dense_results, 60, logger)
        # rrf_threshold = 0.03
        # filtered_rrf_resulst = [r for r in results if r['rrf_score'] >= rrf_threshold]
        logger.info(_log_message(f"[{org_id}] RRF complete {results}", "get_context_from_pinecone", MODULE_NAME))

        # Before rerank logging
        before_rerank_scores = [
            {
                "file_id": r["metadata"]["file_id"],
                "file_name": r["metadata"]["file_name"],
                "rrf_score": r["rrf_score"],
                "query": query,
                "chunk_text": r["metadata"]["text"]
            }
            for r in results
        ]
        logger.debug(_log_message(f"[{org_id}] Before Reranking: {orjson.dumps(before_rerank_scores).decode()}", "get_context_from_pinecone", MODULE_NAME))

        # Rerank
        rerank_start = time.perf_counter()
        reranked_context = rerank_documents(query, results, logger)
        rerank_time = round(time.perf_counter() - rerank_start, 6)

        # Handle both dict and list return types from rerank_documents
        if isinstance(reranked_context, dict) and "matches" in reranked_context:
            matches = reranked_context["matches"]
        elif isinstance(reranked_context, list):
            matches = reranked_context
        else:
            matches = []

        # Apply filtering threshold
        filtered_matches = [
            r for r in matches
            if r.get("score", r.get("rrf_score", 0)) >= 0.30
        ]

        # Wrap filtered list back in dict form
        filtered_reranked_context = {"matches": filtered_matches}

        # Logging after filtering
        after_rerank_scores = [
            {
                "file_id": r["metadata"].get("file_id"),
                "file_name": r["metadata"].get("file_name"),
                "score": r.get("score", r.get("rrf_score")),
                "query": query,
                "chunk_text": r["metadata"].get("text", "")
            }
            for r in filtered_reranked_context["matches"]
        ]

        logger.debug(f"After rerank filtering: {len(after_rerank_scores)} results retained (threshold ≥ 0.30)")

        total_time = round(time.perf_counter() - start_time, 6)
        log_data = {
            "file_id": file_id,
            "user_id": user_id,
            "org_id": org_id,
            "query": query,
            "before_rerank": before_rerank_scores,
            "after_rerank": after_rerank_scores,
            "execution_times": {
                "sparse_query_time": sparse_time,
                "dense_query_time": dense_time,
                "reranking_time": rerank_time,
                "total": total_time
            }
        }
        logger.info(_log_message(f"HYBRID SEARCH PROCESS SUMMARY: {orjson.dumps(log_data).decode()}", "get_context_from_pinecone", MODULE_NAME))

        return filtered_reranked_context

    except Exception as e:
        logger.error(_log_message(f"[{org_id}] Error in hybrid search retrieval: {e}", "get_context_from_pinecone", MODULE_NAME))
        raise RuntimeError(f"Hybrid search failed for Org ID {org_id}: {e}")


@mlflow.trace(name="Get Chunks Text")
def get_chunks_text(context_chunks):
    matches = context_chunks.get('matches', [])
    if not matches:
        return []
    context = [match['metadata']['text'] for match in matches]
    return context